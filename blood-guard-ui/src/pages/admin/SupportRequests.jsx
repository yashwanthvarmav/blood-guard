...
import { getSupportRequests, updateSupportRequest } from "../../context/apis";
...
const SupportRequests = ({ type }) => {
  ...
  const filteredUsers = donorsList?.length
    ? donorsList.filter((user) => {
        const matchesSearch =
          user?.support_name?.toLowerCase().includes(search.toLowerCase()) ||
          user?.support_email?.toLowerCase().includes(search.toLowerCase());
  ...
  const getDonorsList = async () => {
    try {
      ...
      setDonorsList(response?.supports);
    } catch (e) {
      console.log(e);
    }
  };
...
const updateSupport = async (id) => {
    try {
      ...
      const response = await updateSupportRequest(activeUser?.support_id, {   
  ...
  return (
    <div className="p-4 w-full">
      <h1 className="text-2xl font-bold text-red-600 mb-4">Support Requests</h1>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 ">
                <div className="w-full">
                  ...
                  <Select
                    options={['Pending', 'In Progress', 'Resolved', 'Closed']?.map((item) => ({
                      label: item,
                      value: item,
                    }))}
